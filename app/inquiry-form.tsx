...
export function InquiryForm() {
  ...
  return <form action={endpoint} method="post" onSubmit={submit} aria-busy={status === 'sending'}>
    <input type="hidden" name="inquiry_type" value="Inner Temple question"/>
    <input type="hidden" name="_subject" value="New Sparkle Like Zen Inner Temple Question"/>
    <label>Name<input name="name" autoComplete="name" required/></label>
    <label>Email<input name="email" type="email" autoComplete="email" required/></label>
    <label>Message<textarea name="message" rows={5} required/></label>
    <button className="button button-accent" type="submit" disabled={status === 'sending'}>{status === 'sending' ? 'Sending…' : 'Send a Message'}</button>
    <div className="form-status" aria-live="polite">
      {status === 'success' && <p className="form-success">Thank you. Your message has been sent, and I’ll be in touch soon.</p>}
      {status === 'error' && <p className="form-error">Something went wrong while sending your message. Please try again or email info@sparklelikezen.com.</p>}
    </div>
  </form>;
}
